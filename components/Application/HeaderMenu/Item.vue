...
<template>
    <template v-if="props.item.children.length > 0">
        <HeadlessMenu as="div" class="relative">
            <HeadlessMenuButton :class="['group relative overflow-hidden flex items-center px-2 py-3 cursor-pointer duration-150 hover:text-primary']" aria-label="Header Menu Dropdown">
            ...
            <TransitionSlide>
                <HeadlessMenuItems class="absolute top-full right-0 mt-4 bg-white rounded-lg border shadow-md py-2 pl-4 w-52 divide-y divide-dashed">
                    <HeadlessMenuItem v-for="subItem in item?.children" :key="subItem.id" class="relative first:pt-0 last:pb-0 py-2">
                        <NuxtLink :to="'/' + subItem.link" class="dark:hover:text-primary hover:text-primary/75 hover:pl-4 relative overflow-hidden flex items-center cursor-pointer duration-300">
