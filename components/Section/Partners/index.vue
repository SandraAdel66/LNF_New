    ...
    <div>
        ...
        <div v-if="status !== 'pending'" class="grid lg:grid-cols-4 gap-8">
            <template v-for="item in partners as Partner[]">
                <NuxtLink v-if="item.active" :key="item.id" :href="item.link" class="bg-white rounded-xl ring !ring-slate-400/10 hover:scale-105 transition-all p-8" target="_blank">
                    <NuxtImg :alt="item.name" :src="item.imageUrl" :title="item.name" class="h-full mx-auto object-contain bg-white" height="100%" loading="lazy" preload quality="80" width="100%" />
                </NuxtLink>
            </template>
        </div>
    </div>
